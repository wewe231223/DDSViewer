...
bool ViewerApplication::InitializeD3D12() {
    ...
    mSourceCpuHandle = mSrvHeap->GetCPUDescriptorHandleForHeapStart();
    mSourceGpuHandle = mSrvHeap->GetGPUDescriptorHandleForHeapStart();
...
bool ViewerApplication::InitializeImGui() {
    ...
    if (!ImGui_ImplDX12_Init(mDevice.Get(), FrameCount, DXGI_FORMAT_R8G8B8A8_UNORM, mSrvHeap.Get(), mSourceCpuHandle, mSourceGpuHandle)) {
        return false;
    }
    return true;
}
...
void ViewerApplication::HandleDroppedFile(HDROP DropHandle) {
    ...
    if (mAnalyzer.LoadTexture(FilePath)) {
